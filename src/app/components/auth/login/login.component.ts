...
export class LoginComponent extends CommonComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.isDemoVersion) {
      this.loginImg = 'assets/images/Artboard_full.png';
      this.welcomString = '';
    }
  }
      ...
      this.authService.authenticate(this.userObject).subscribe({
        next: (result: any) => {
          if (result) {
            ...
            this.apiService.GetAllMenuItems(result.emailAddress, result.userSessionId).subscribe({
              next: (results) => {
                this.navState = this.navService.getNavigationState();
                this.navState.menuGroups = results;
